<template>
  <div>
    ...
    <button v-if="saveUrl != null && saveUrl != '' && delUrl != null && delUrl != '' " class="btn btn-primary btn-sm"
            type="button" style="margin-right: 20px;" @click="addRow()">添加
    </button>
    <hr style="opacity: 0;border-style: none;"/>
    <div class="table-responsive">
      <table class="table table-sm table-striped">
        <thead>
        <tr>
          <th v-for="(column,index) in columns" :key="index">{{ doFieldMap(column) }}</th>
...
export default {
  props: ["dataUrl", "saveUrl", "delUrl", "data", "fields", "fieldMap"],
  data() {
    return {
      columns: {},
      lines: {},
  ...
  methods: {
    ...
    validateImage: function (url) {
      if (url == null) {
        ...
        return url.match(/\.(jpg|png|gif|webp)/g)
      }
    },
    queryData: function () {
      const self = this;
      axios
          .post(self.$props.dataUrl, self.$props.data, {
            headers: {
              'Content-Type': 'application/json'
            }
          })
          .then(function (response) {
            let tableDataList = response.data.data.list;
    ...
    doFieldMap: function (field) {
      if (this.$props.fieldMap != null && this.$props.fieldMap[field] !== undefined) {
        return this.$props.fieldMap[field]
      } else {
        return field
      }
    }
